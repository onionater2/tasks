...
subID = input('Enter target name: ','s');
...
subDir = sprintf('%s%s%s',studyDir,'self/',subID);
scaleImgDir = sprintf('%sscale_images',studyDir);

fprintf('Moving to study directory for subject %s.\n\n',subID)
cd(subDir);
...
while correctOrder==false;
    ...
    fprintf('\n\nThe following movies have been located in the current subject directory:\n')
    ...
    queryOrderInput = input('The trials will show movies in the order above.  Is this correct [y/n]? ','s');
    if strcmpi(queryOrderInput, 'Y') || strcmpi(queryOrderInput, 'YES')
        correctOrder=true;
    else
        fprintf('\nPlease rename the video files in ascending numerics based on the desired order of presentation.\n')
        fprintf('Press any key while this window is active once the files have been renamed...\n\n')
        pause;
    end
end

%% Create the structure for recording responses from the experiment:
subjectStructName=sprintf('subject%s',subID);

% Overwrite check:
for i = 1:numberTrials
    currentTrial=num2str(i);
    outputFilename = sprintf('subject%s_phase1_trial%s_output.txt',subID,currentTrial);
...
try
    ...
    instructText1 = 'RATING YOUR OWN VIDEOS';  
    instructText2 = 'Please continuosly rate how you were feeling WHILE YOU WERE RECORDING EACH VIDEO. Do NOT rate how you were feeling at the time of the event.';
    instructText3 = 'Use the scale from 1 (very negative) to 9 (very positive) to rate how you were feeling in each moment throughout the entire video.' 
    instructText4 = 'It is especially important that you move the scale EACH TIME your feelings in the video change in a MAJOR WAY.';
...
k=waitforbuttonpress;
    ...
    while i<=numberTrials
        ...
        [movie movieDur fps movie_imgw movie_imgh] = Screen('OpenMovie', win, [subDir, '/', movieList{i}]);
